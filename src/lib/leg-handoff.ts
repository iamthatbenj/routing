...
export function routeGeometryWarning(savedRoute: SavedRoute) {
  ...
  if (hasHandoffStops) {
    return 'This Saved Route uses visible Shaping Stops to encourage external navigation apps to follow the intended Corridor. Google Maps receives those stops as provider waypoints; Apple Maps web links open Endpoints only, so add Shaping Stops manually there. Confirm the full route before driving.';
  }

  return isFastest
    ? 'External navigation apps may still change the intended Corridor because of traffic, closures, or app settings.'
