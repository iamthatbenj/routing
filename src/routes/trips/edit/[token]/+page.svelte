          ...
          <li>
            ...
            <div class="stop-controls">
              ...
              <form class="delete-stop" method="POST" action="?/deleteStop" aria-label={`Delete ${stop.routingPlace.name}`} onsubmit={confirmTripStopDelete}>
                <input type="hidden" name="stopId" value={stop.id} />
                <button type="submit" aria-describedby={`delete-warning-${stop.id}`}>
                  Delete
                </button>
                <p id={`delete-warning-${stop.id}`}>Deleting this Trip Stop can remove or change adjacent Legs and their Route Search or Saved Route context.</p>
              </form>
            </div>
          </li>
        {/each}
